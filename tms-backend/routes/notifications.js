...
module.exports = function(io) {
  ...
  io.on('connection', (socket) => { 
    ...
    router.patch("/:notificationId", async (req, res) => {
      const updated_notification = await Notification.updateOne({ _id: req.params.notificationId }, { $set: {[req.body.attr]: req.body.value} })
        .then((data) => {
          res.json(data);
        })
        .catch((err) => {
    ...
    router.get("/all/:userId", async (req, res) => {
      await Notification.find({receiver: req.params.userId}).sort({date:-1})
        .then((data) => {
          res.json(data);
        })
        .catch((err) => {
    ...
    router.get("/some/:userId", async (req, res) => {
      await Notification.find({receiver: req.params.userId}).sort({date:-1}).limit(20)
        .then((data) => {
          res.json(data);
        })
        .catch((err) => {
