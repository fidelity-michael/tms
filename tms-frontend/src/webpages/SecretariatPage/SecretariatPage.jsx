    ...
    const authUser = async () => {
      try {
        ...
        if (auth_data.data.auth) {
          if(
            (location.state === "changeRole" && auth_data.data.role.includes("secretariat")) ||  //we come from my roles page
            (location.state ==="login" && auth_data.data.role[0] === "secretariat")     //we come from login
          ){
        ...
        <div id="content-wrapper" className="d-flex flex-column">
          <div id="content">
            ...
            <div className="container-fluid">
              ...
              {(page === "Dashboard") ? <SecretariatDashboard userId={userId} setPage={setPage}/> : null}
              {(page === "Active Theses") ? <ActiveThesesTable userId={userId} userGroup={"Secretariat"} /> : null}
              {(page === "Completed Theses") ? <CompletedTheses userId={userId} /> : null}
              {(page === "Theses Archive") ? <ArchivedTheses userId={userId} /> : null}
              {(page === "My Roles") ? <MyRoles userId={userId}  currentRole="secretariat"/> : null}
