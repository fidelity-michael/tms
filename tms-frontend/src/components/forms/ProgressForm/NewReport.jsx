...
export default function NewReport({ userId, thesisCompleted, onSubmitThesis, thesisData, email}) {
    ...
    function renderReportFiles() {
        ...
        return(
          reportFiles.map((fileList, index) => {
            for(let i=0; i<fileList.length; i++){ //multiple
              return (
                <div className="newThesisFile" key={fileList[i].name+index}>
                  <i className='fa fa-trash-alt' type="button" onClick={() => removeReportFile(fileList[i].name)}></i>
    ...
    return (
        <Form onSubmit={(e) => submitReport(e)}>
            ...
            {
                report.isFinal ?
                    <span style={{ color: "red", textAlign: "center" }}>Caution: If you submit thesis you will not be able to upload more reports!</span>
                    : null
            }
