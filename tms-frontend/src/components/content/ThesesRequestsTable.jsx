...
export default function ThesesRequestsTable({ userId, email, thesesRequest, requestAction }) {
  ...
  function sendNotification(target, action) {
    ...
    const notifyStudent = async () => {
      await axios.post('/notifications', {
        ...
        message: "Request for " + thesisTitle + " " + action + ".\nProfessor: " + email,
        receiver: studentId,
        type: "info"
      })
        .then(res => {
    ...
    if (find_request) {
      ...
      return (
        <div className="btn-group" role="group" aria-label="Button group with nested dropdown">
          ...
          />
          <button type="button" data-key={studentId} id={id} name={thesisTitle} className="btn btn-success accept-request" onClick={(e) => handleAcceptClicked(e.target)}>Accept</button>
          <button type="button" data-key={studentId} id={id} name={thesisTitle} className="btn btn-danger decline-request" onClick={(e) => handleDeclineClicked(e.target)}>Decline</button>
        </div>
      );
    }
  }
  ...
  function renderRequestsData() {
    ...
    if (filtered_requests.length) {
      ...
      return filtered_requests.map((request) => {
        const { _id, date, thesis_title, thesis_topic, thesis_area, thesis_group, student_id, student_email, student_name, required_files } = request;
        return (
          <tr key={_id}>
            ...
            <td className='table-data'>
              {
                renderDownloads(required_files)
              }
            </td>
        ...
        <Table className='requests-table' striped bordered hover size="md" responsive>
          <thead>
            <tr>
              ...
              <th className='table-header'><span id='student_email' onClick={(e) => toggleOrder(e.target.id)}>Student Email</span></th>
              <th className='table-header'><span id='student_files' onClick={(e) => toggleOrder(e.target.id)}>Student Files</span></th>
              <th className='table-header'><span id='action'>Request Action</span></th>
            </tr>
          </thead>
          <tbody>
            {
