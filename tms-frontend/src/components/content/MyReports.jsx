...
export default function ReportsArchive({ userId, email, user }) {
  ...
  function downloadFile(target) {
    ...
    const fetchData = async () => {
      ...
      await axios
        .get("/api/data/downloads/reports/" + target.name, {
          responseType: "blob",
        })
        .then((res) => {
  ...
  function sendComment(index, reportTitle, reportId) {
    ...
    const notifyStudent = async () => {
      await axios
        .post("/notifications", {
          ...
          message:
            email + ": " + document.getElementById("comment_" + index).value,
          receiver: studentId,
          type: "info",
        })
        .then((res) => {
          ...
          setTimeout(function () {
            if (document.getElementById("info_" + index))
              document.getElementById("info_" + index).innerHTML = "";
          }, 2000);
        })
        .catch((err) => {
    ...
    const addCommenttoDB = async () => {
      await axios
        .patch("/api/theses_reports/addComment/" + reportId, {
          attr: "comments",
          value:
            email + ": " + document.getElementById("comment_" + index).value,
        })
            ...
            <div className="collapseWrap">
              <div className="collapse" id={"collapse" + index}>
                ...
                <button
                  ...
                  style={{ display: "none" }}
                  onClick={() => sendComment(index, title, _id)}
                >
                  Send
                </button>
              </div>
            ...
            <div className="collapse" id={"collapse" + index}>
              {report_files[0] && (
                <div className="download-files-list">
                  ...
                  <ul>{renderDownloads(report_files)}</ul>
                </div>
              )}
              <div className="date-posted">
                <span>Date of submission: </span>
  ...
  function loadingData() {
    return (
      <div
        className="d-flex justify-content-center hover:tw-bg-light-pale-blue-white tw-text-dark-sky-blue tw-placeholder-dark-sky-blue"
  ...
  function emptyData(e) {
    return (
      <h5
        ...
        colSpan={100}
      >
        There are no reports uploaded yet
      </h5>
    );
  }
  ...
  return (
    ...
    >
      ...
      <PaginationTab
        setLimit={(e) => {
          setReportsLimit(e.target.value);
        }}
        renderPageButtonsName="report"
        pagination={pagination}
        setPageState={setReportsPage}
      />
    </div>
  );
}
