...
export default function ArchivedTheses({ userId }) {
  ...
  async function updateAssignedThesis(id){
      ...
      const thesisTitleEnglish = document.getElementById(id+"thesisTitleEnglish")
      ...
      //updates
      if(thesisTitleGreek.value && (thesisTitleGreek.value !== thesisTitleGreek.placeholder)){
        flag = 1
        updateAttribute(id, "title_greek", thesisTitleGreek.value)
      }

      if(thesisTitleEnglish.value && (thesisTitleEnglish.value !== thesisTitleEnglish.placeholder)){
        flag = 1 
  ...
  function renderThesesData() {
    ...
    if (filtered_theses.length) {
      ...
      return filtered_theses.map((find_thesis, index) => {
        ...
        const { _id, date, thesis_title_greek, thesis_title_english, thesis_group, student_id, student_email, student_name, professor_email, professor_name, thesis_grade } = find_thesis;
        return (
          <tr key={_id}>
            ...
            <td className='table-data'>
              <input type="text" id={_id+"thesisTitleGreek"} name="title_greek" data-key={_id} className="editable-data" placeholder={thesis_title_greek} size={thesis_title_greek.length + 1} autoComplete="off" />
            </td>
            <td className='table-data'>
              <input type="text" id={_id+"thesisTitleEnglish"} name="title_english" data-key={_id} className="editable-data" placeholder={thesis_title_english} size={thesis_title_english.length} autoComplete="off" />
            </td>
            <td className='table-data' id={_id+"group"}>{thesis_group}</td>
            <td className='table-data' id={_id+"professorEmail"} data-toggle="tooltip" data-placement="right" title={professor_name}>{professor_email}</td>
            <td className='table-data' id={_id+"studentName"} data-toggle="tooltip" data-placement="right" title={student_name}>{student_email}</td>
            <td className='table-data'>
              <input type="number" id={_id+"grade"} step="0.25" min="0" max="10" name="grade" data-key={_id} className="editable-data" placeholder={thesis_grade} size={thesis_grade.length} autoComplete="off" />
            </td>
            <td className='table-data'>
              <button type="button" data-key={_id} id={student_id} className="btn btn-info accept-request" 
                      style={{width: "10vw"}} onClick={() => updateAssignedThesis(_id)} 
              >
                Update
              </button>
              <div style={{marginTop: "1.3vh"}} >
                <b><small className="infoUpdateUser" id={_id+"info"} ></small></b>
              </div>
            </td>
          </tr>
        )
        ...
        <Table className='theses-table' striped bordered hover size="md" responsive>
          <thead>
            <tr>
              ...
              <th className='table-header'><span id='title_1' onClick={(e) => toggleOrder(e.target.id)}>Thesis Greek Title</span><i className="fa fa-edit edit-input-icon"></i></th>
              <th className='table-header'><span id='title_2' onClick={(e) => toggleOrder(e.target.id)}>Thesis English Title</span><i className="fa fa-edit edit-input-icon"></i></th>
              <th className='table-header'><span id='group' onClick={(e) => toggleOrder(e.target.id)}>Group</span></th>
              <th className='table-header'><span id='professor' onClick={(e) => toggleOrder(e.target.id)}>Professor</span></th>
              <th className='table-header'><span id='student' onClick={(e) => toggleOrder(e.target.id)}>Student</span></th>
              <th className='table-header'><span id='grade' onClick={(e) => toggleOrder(e.target.id)}>Grade</span><i className="fa fa-edit edit-input-icon"></i></th>
