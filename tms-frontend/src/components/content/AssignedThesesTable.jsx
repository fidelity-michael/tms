...
export default function AssignedThesesTable({ userId }) {
  ...
  function handleSupervisorChange(target, index) {
    ...
    const supervisor_index = supervisors.findIndex(supervisor => supervisor.email === target.value);
    ...
    console.log('Selected professor as new supervisor: ', updateSupervisor[index]);    
  }
  ...
  function handleProposeSupervisor(target, student_id, index, emails) {
    ...
    if(supervisors.length === emails.length)
      return
    ...
    if (updateSupervisor[index]===undefined) {  //prwtos aptous potential giati den diallexame kapoion
      newSupervisorId=supervisors.find((supervisor) => !emails.includes(supervisor.email))
    ...
    newSupervisorsIdArray.filter(element => element!==null && element!==undefined);
    ...
    if (supervisors.length) {
      ...
      return supervisors.map((supervisor) => {
        ...
        if(!supervisors_emails.includes(supervisor.email)){
          return (
            <option id={_id} key={_id}>{email}</option>
          )
        } else {
          return null
        }
      ...
      return (
        <option key={0}>No supervisors found</option>
      )
    }
  }
  ...
  function renderThesesData() {
    ...
    if (filtered_theses.length) {
      ...
      return filtered_theses.map((find_thesis, index) => {
        if (find_thesis.thesis_status === "graded" || find_thesis.thesis_status === "archived") {
          const data = {
        ...
        const { _id, date, thesis_title, thesis_topic, thesis_area, thesis_group, student_id, student_email, student_name, supervisor_email, thesis_status } = find_thesis;
        
        //list all supervisors
        const listSupervisorsEmails =
          <ul>
            {supervisor_email.map(email => 
            <li key={thesis_title+'_'+email}>{email} 
              <i className='fa fa-trash-alt' type="button" id={email} style={{color: '#ec2020'}} onClick={(e) => removeSupervisor(e, index)}></i>
            </li>)}
          </ul>
        ...
        return (
          <tr key={_id}>
            ...
            <td className='table-data' data-toggle="tooltip" data-placement="right">{listSupervisorsEmails}</td>
            <td className='table-data'>
            {
              thesis_status === "active" ? <span>{thesis_status}</span> :
              <span id={student_id} className="modal-item"
                onClick={(e) => { setTitle(thesis_title); 
            ...
            <td className='table-data'>
              <i id={student_id} className="modal-item far fa-file-alt fa-lg" 
                style={{fontSize: "4vh"}}
                onClick={(e) => { setTitle(thesis_title); 
            ...
            <td className='table-data'>
              {
                <div className="btn-group" role="group" aria-label="Button group with nested dropdown">
                  <Form.Group>
                    <Form.Control as="select" id={_id} data-key={index} className='supervisors-dropdown' value={updateSupervisor[index]} onChange={(e) => handleSupervisorChange(e.target, index)} >
                      {
                        renderPotentialSupervisors(supervisor_email)
                      }
                    </Form.Control>
                  </Form.Group>
                  <button type="button" id={_id} className="btn btn-info propose-btn" onClick={(e) => handleProposeSupervisor(e.target, student_id, index ,supervisor_email)}>Add</button>
                </div>
              }
            </td>
          </tr>
  ...
  return (
    <div className='tables-data'>
      {/* Modals */}
      <ReportsModal title={title} userId={student} show={showReportModal} onShow={(data) => setShowReportModal(data)} />
      <GradingModal title={title} userId={student} completedData={gradedData} onCompleted={(data) => setCompletedData(data)} show={showGradingModal} onShow={(data) => setShowGradingModal(data)} />
        ...
        <Table className='theses-table' striped bordered hover size="md" responsive>
          <thead>
            <tr>
              ...
              <th className='table-header'><span id='thesis_status' onClick={(e) => toggleOrder(e.target.id)}>Status</span></th>
              <th className='table-header'><span id='reports' onClick={(e) => toggleOrder(e.target.id)}>Reports</span></th>
              <th className='table-header'><span id='action'>Add Supervisor</span></th>
            </tr>
          </thead>
          <tbody>
            {
