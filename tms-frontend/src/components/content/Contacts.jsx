...
export default function Contacts({myContacts, conversations, hideNotifyIcon, contactSelectStyle, switchContact,  checkLastMessageRead}) {
    ...
    function renderContacts(){
        ...
        if(myContacts.length>0){
            ...
            var contactsIds = myContacts.map((contact) => {return contact._id})
            //console.log('contactsIds',contactsIds, myContacts)
    
            conversations.forEach((conversation, index) => {
                if(!contactsIds.includes(conversation.user1) && !contactsIds.includes(conversation.user2))
            ...
            sorted.forEach((conversation, index) => {
                var user = myContacts.find(user => user._id === conversation.user1 || user._id === conversation.user2)
            ...
            if(sorted.length>0){
                ...
                myContacts.forEach((user) => {
                    var index = sorted.findIndex(conversation => user._id === conversation.user1 || user._id === conversation.user2)
                    
                    if(index<0)
                        notChatted.push(user)
                })
            } else {
            ...
            return contacts.map((contact, index) => {
                if(index<sorted.length){ //sorted
    
                    //console.log(index)
                    
                    return(
                        <div key={contact.chatId} id={"contact"+contact.chatId}>
                            <li className="contact list-group-item" type="button" 
                            ...
                            >
                                <i className="fas fa-circle" id={"icon"+contact.chatId} 
                                    style={{display: checkLastMessageRead(contact.lastMessage) ? "none" : "block"}}
                                >
                                </i>
                ...
                } else { //notChatted (ctually user objects)
                    //console.log(index)
