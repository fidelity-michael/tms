...
export default function UsersTable() {
    ...
    async function updateUser(id){
        ...
        if(firstName) { 
            flag = 1
            //change firstname
            if(firstName !== users[index].first_name) await updateUsersAttribute(id, "first_name", firstName)
        } 

        if(lastName) {
            flag = 1
            //change lastname
            if(lastName !== users[index].last_name) await updateUsersAttribute(id, "last_name", lastName)
        } 
        ...
        if(JSON.stringify(newRolesArray.sort()) !== JSON.stringify(users[index].role.sort()) ){
            flag=1
        ...
        if(flag === 0){
            ...
            setVariant("success")
            setMessage("Successful Update of user: "+users[index].first_name+" "+users[index].last_name+".");
            setShowAlert(true);
    ...
    function renderUsersData() {
        ...
        if (filtered_users.length) {
            ...
            return filtered_users.map((user, index) => {
                const { _id, first_name, last_name, email, role, /*group,*/ status } = user;
                ...
                return (
                    <tr key={_id}>
                        ...
                        <td className='table-data'>
                            <input type="text" name="first_name" id={_id+"first_name"} data-key={_id} className="editable-data" placeholder={first_name} size={first_name.length} autoComplete="off" />
                        </td>
                        <td className='table-data'>
                            <input type="text" name="last_name" id={_id+"last_name"} data-key={_id} className="editable-data" placeholder={last_name} size={last_name.length} autoComplete="off" />
                        </td>
                        ...
                        <td className='table-data'>
                            ...
                            <div id={_id+"roleList"} className="dropdown-check-list">
                                <span className="anchor" onClick={() => dropDownRoles(_id, role)}>Edit Roles</span>
                                <ul className="roles" data-key={_id} >
                                    <li className="roleListOption">
                                        <label htmlFor="professor">Professor</label>
                                        <input type="checkbox" id={_id+"professor"} defaultChecked={isProfessor} onChange={()=>{/*for warning*/}} />
                                    </li>
                                    <li className="roleListOption">
                                        <label htmlFor="administrator">Administrator</label>
                                        <input type="checkbox" id={_id+"administrator"} defaultChecked={isAdmin} onChange={()=>{/*for warning*/}} />
                                    </li>
                                    <li className="roleListOption">
                                        <label htmlFor="student">Student</label>
                                        <input type="checkbox" id={_id+"student"} defaultChecked={isStudent} onChange={()=>{/*for warning*/}} />
                                    </li>
                                    <li className="roleListOption">
                                        <label htmlFor="secratariat">Secretariat</label>
                                        <input type="checkbox" id={_id+"secretariat"} defaultChecked={isSecretariat} onChange={()=>{/*for warning*/}} />
                                    </li>
                                </ul>
                            </div>
                        ...
                        <td className='table-data'>
                            ...
                            <select className="editable-data" name="status" data-key={_id} id={_id+"status"} defaultValue={status} >
                        ...
                        <td className='table-data'>
                            <div className="btn-group" role="group" aria-label="Button group with nested dropdown">
                                <button type="button" data-key={_id} className="btn btn-info accept-request" onClick={(e) => updateUser(_id)} >Update</button>
                                <button type="button" name={email} data-key={_id} className="btn btn-danger decline-request" onClick={(e) => handleCellDelete(e.target)} >Delete</button>
                            </div>
                            <div>
                                <b><p className="infoUpdateUser" id={_id+"info"}></p></b>
                            </div>
                        </td>
                    </tr>
                )
    ...
    return (
        <div className='tables-data'>
            ...
            <div className='users-container'>
                ...
                <Table className='users-table' striped bordered hover size="md" responsive>
                    <thead>
                        <tr>
                            <th>#</th>
                            <th className='table-header'><span id='first_name' onClick={(e) => toggleOrder(e.target.id)}>First Name</span><i className="fa fa-edit edit-input-icon"></i></th>
                            <th className='table-header'><span id='last_name' onClick={(e) => toggleOrder(e.target.id)}>Last Name</span><i className="fa fa-edit edit-input-icon"></i></th>
                            <th className='table-header'><span id='email' onClick={(e) => toggleOrder(e.target.id)}>Email</span></th>
                            <th className='table-header'><span id='role' onClick={(e) => toggleOrder(e.target.id)}>Role</span><i className="fa fa-edit edit-input-icon"></i></th>
                    ...
                    <tbody>
                        {
                            loadingUsers ? loadingTable() : (users.length ? renderUsersData() : emptyTable())
                        }
                    </tbody>
                </Table>
                {
                ...
                <div className='dropdown-limit'>
                    <Form.Group controlId="selectControl">
                        ...
                        <Form.Control className='page-limit' as="select" onChange={(e) => { setUsersLimit(e.target.value); }}>
                            <option>10</option>
                            <option>25</option>
                            <option>50</option>
                        </Form.Control>
